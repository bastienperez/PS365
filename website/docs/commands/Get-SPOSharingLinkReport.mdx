---
description: Lists every sharing link of one or more SharePoint Online sites, one row per link, using PnP.PowerShell app-only.
document type: cmdlet
HelpUri: https://ps365.clidsys.com/docs/commands/Get-SPOSharingLinkReport
Locale: en-US
Module Name: PS365
ms.date: 09/07/2026
...
```
Get-SPOSharingLinkReport [-SiteUrl] <string[]> -ClientId <string> -Tenant <string>
 [-CertificateThumbprint <string>] [-CertificatePath <string>] [-CertificatePassword <securestring>]
 [-CertificateBase64Encoded <string>] [-IncludeGraphDetails] [-LinkType <string[]>] [-ExpiredOnly]
 [-NeverExpiringOnly] [-ExpiringInDays <int>] [-ThrottleLimit <int>] [-ExportToExcel]
 [-ExportPath <string>]
```
