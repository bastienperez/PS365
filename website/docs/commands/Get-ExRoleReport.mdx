---
description: Reports on Exchange RBAC roles and their effective membership, including groups expanded recursively.
...
```powershell
Get-ExRoleReport [[-Identity] <string>] [[-PrincipalID] <string>]
 [[-PrincipalDisplayName] <string>] [-OnPrem]
```
