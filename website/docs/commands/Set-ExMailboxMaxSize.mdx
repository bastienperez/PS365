...
```powershell
Set-ExMailboxMaxSize [-Identity] <string[]> [-GenerateCmdlets]
...
```powershell
Set-ExMailboxMaxSize -ByDomain <string> [-GenerateCmdlets]
 [-OutputFile <string>] [-WhatIf] [-Confirm]
```
...
The path to a CSV file with a PrimarySmtpAddress column.
All rows are validated and resolved before any change.
