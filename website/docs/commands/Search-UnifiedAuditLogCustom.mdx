---
description: Search-UnifiedAuditLogCustom is an enhanced wrapper around the native Search-UnifiedAuditLog cmdlet, providing additional features such as a user-friendly GUI for constructing search queries, simplified output formatting, and integration with the Microsoft 365 audit operations catalog.
document type: cmdlet
HelpUri: https://ps365.clidsys.com/docs/commands/Search-UnifiedAuditLogCustom
Locale: en-US
Module Name: PS365
ms.date: 09/08/2026
sidebarTitle: Search-UnifiedAuditLogCustom
...
```powershell
Search-UnifiedAuditLogCustom [[-StartDate] <datetime>] [[-EndDate] <datetime>]
 [[-Operations] <string[]>] [[-UserIds] <string[]>] [[-FreeText] <string>]
 [[-ResultSize] <int>] [[-ChunkDays] <int>] [-SimpleView] [-HelperGUI]
```
