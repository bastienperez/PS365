---
description: Retrieves all Entra ID applications configured for SCIM provisioning.
document type: cmdlet
HelpUri: https://ps365.clidsys.com/docs/commands/Get-MgApplicationSCIM
Locale: en-US
Module Name: PS365
ms.date: 09/07/2026
sidebarTitle: Get-MgApplicationSCIM
...
```powershell
Get-MgApplicationSCIM [-ExcludeAttributeMappings] [-IncludeFailedObjects]
 [-ForceNewToken] [-ExportToExcel] [-ExportPath <string>]
 [-RunFromAzureAutomation] [-NotificationRecipient <string>]
 [-NotificationSender <string>] [-DisableParallel] [-ThrottleLimit <int>]
 [-UseBatchRequest] [-EntraCloudSync]
```

### ByObjectId

```powershell
Get-MgApplicationSCIM [-ObjectID <string>] [-ExcludeAttributeMappings]
 [-IncludeFailedObjects] [-ForceNewToken] [-ExportToExcel]
 [-ExportPath <string>] [-RunFromAzureAutomation]
 [-NotificationRecipient <string>] [-NotificationSender <string>]
 [-DisableParallel] [-ThrottleLimit <int>] [-UseBatchRequest] [-EntraCloudSync]
```

### ByDisplayName

```powershell
Get-MgApplicationSCIM [-DisplayName <string>] [-ExcludeAttributeMappings]
 [-IncludeFailedObjects] [-ForceNewToken] [-ExportToExcel]
 [-ExportPath <string>] [-RunFromAzureAutomation]
 [-NotificationRecipient <string>] [-NotificationSender <string>]
 [-DisableParallel] [-ThrottleLimit <int>] [-UseBatchRequest] [-EntraCloudSync]
...
Requires the PS365 module (Invoke-MgGraphBatchRequest); when the command is not
available (e.g.
script deployed standalone), the function warns and falls back to the default behavior.
