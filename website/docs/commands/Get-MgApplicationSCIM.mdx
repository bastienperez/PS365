...
Get-MgApplicationSCIM [-ExcludeAttributeMappings] [-IncludeFailedObjects] [-ForceNewToken] [-ExportToExcel]
 [-RunFromAzureAutomation] [-NotificationRecipient <String>] [-NotificationSender <String>] [-Parallel]
...
Get-MgApplicationSCIM [-ObjectID <String>] [-ExcludeAttributeMappings] [-IncludeFailedObjects] [-ForceNewToken]
 ...
 [-Parallel] [-ThrottleLimit <Int32>] [-EntraCloudSync] [-ProgressAction <ActionPreference>]
 [<CommonParameters>]
```
...
Get-MgApplicationSCIM [-DisplayName <String>] [-ExcludeAttributeMappings] [-IncludeFailedObjects]
 ...
 [-NotificationSender <String>] [-Parallel] [-ThrottleLimit <Int32>] [-EntraCloudSync]
...
Scans service principals in parallel (8 concurrent runspaces) to speed up discovery.
...
Requires PowerShell 7 or later (uses ForEach-Object -Parallel).
Ignored/blocked on PowerShell 5.1.
...
(Optional) Maximum number of concurrent runspaces when -Parallel is used.
Default is 5.
