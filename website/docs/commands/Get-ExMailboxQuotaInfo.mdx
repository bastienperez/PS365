...
```
Get-ExMailboxQuotaInfo [[-Identity] <string[]>] [-OnlyIssues] [-ForceNewToken] [-ExportToExcel]
 [-ExportPath <string>]
```
...
It computes the expected maximum quota from the licenses (100 GB for BPOS_S_ENTERPRISE or for a
Business suite with the storage add-on, 50 GB for BPOS_S_STANDARD alone such as standalone
Exchange Online Plan 1, 2 GB for EXCHANGE_S_DESKLESS) and flags the gaps in the Issue column:
...
Quotas versus license entitlement for every user mailbox of the tenant.
...
When omitted, every user mailbox is evaluated.
...
- Issue                    : StorageAddOnNotProvisioned, QuotaBelowEntitlement, or empty
