<#
	...
	.PARAMETER FromCSV
	The path to a CSV file with a PrimarySmtpAddress column. All rows are validated and resolved before any change.
	...
	.EXAMPLE
	Set-ExMailboxMaxSize -Identity "user@example.com"
...
function Set-ExMailboxMaxSize {
    ...
    process {
        ...
        elseif ($PSCmdlet.ParameterSetName -eq 'FromCSV') {
            ...
            $csvRows = @(Import-Csv -LiteralPath $FromCSV -ErrorAction Stop)
            if ($csvRows.Count -eq 0) { throw 'The CSV contains no mailbox rows.' }
            foreach ($row in $csvRows) {
                if ([string]::IsNullOrWhiteSpace($row.PrimarySmtpAddress)) {
                    throw 'Every CSV row must have a nonempty PrimarySmtpAddress.'
                }
            }
            foreach ($row in $csvRows) {
                $resolved = @(Get-Mailbox -Identity $row.PrimarySmtpAddress.Trim() -ErrorAction Stop)
                if ($resolved.Count -ne 1 -or -not $resolved[0].PrimarySmtpAddress) {
                    throw "Expected exactly one mailbox for '$($row.PrimarySmtpAddress)'."
                }
                $Mailboxes.Add($resolved[0])
            }
        }
        ...
        foreach ($mbx in $Mailboxes) {
            ...
            $fullCommand = "$cmdletString '$(([string]$cmdParams.Identity).Replace("'", "''"))' $cmdParamstring"
            if ($GenerateCmdlets) {
                $commands.Add($fullCommand)
            }

            if (-not $GenerateCmdlets -and $PSCmdlet.ShouldProcess($mbx.PrimarySmtpAddress, 'Set message send and receive limits to 150 MB')) {
