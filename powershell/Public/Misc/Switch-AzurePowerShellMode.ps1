...
function Switch-AzurePowerShellMode {
    ...
    Switch-PS365WamAuthMode -Label 'Azure PowerShell' `
        -GetState { [bool]((Get-AzConfig -EnableLoginByWam).Value) } `
        -SetState { param($enabled) $null = Update-AzConfig -EnableLoginByWam $enabled } `
